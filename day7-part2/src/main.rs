...
fn get_num_bags_in(bag_color: &str, bag_map: &HashMap<String, Vec<(String, u32)>>) -> Option<u32> {
    let mut total_num: u32 = 0;

    if bag_map[bag_color].is_empty() {
        // break recursion
        return None;
    } else {
        for (color, num) in &bag_map[bag_color] {
            total_num += num;
            if let Some(inner_num) = get_num_bags_in(&color, &bag_map) {
                total_num += num * inner_num;
            }
        }
    }
...
fn main() {
    ...
    if let Some(bags_inside) = get_num_bags_in(check_color, &bag_map) {
        println!(
            "\nTotal Number of bags in {} bag : {}",
            check_color, bags_inside
        );
    } else {
        println!("\nTotal Number of bags in {} bag : 0", check_color);
    }
}
