...
fn main() {
    ...
    let mut init_state: HashMap<(i32, i32), Vec<Vec<char>>> = HashMap::new();
    ...
    init_state.insert((0, 0), init_slice);
...
fn get_next_state(
    old_state: &mut HashMap<(i32, i32), Vec<Vec<char>>>,
...
fn get_new_slice(
    z: (i32, i32),
    mutated_old_state: &HashMap<(i32, i32), Vec<Vec<char>>>,
) -> Vec<Vec<char>> {
    ...
    for i in 0..mutated_old_state[&z].len() {
        for j in 0..mutated_old_state[&z][i].len() {
            let count = count_active_neighbours((i, j, z.0, z.1), &mutated_old_state);
            if let Some(c) = state.get(i, j, z.0, z.1) {
...
fn mutate_old_state(old_state: &mut HashMap<(i32, i32), Vec<Vec<char>>>) {
    let z_min: i32 = *old_state.keys().map(|(z, _w)| z).min().unwrap();
    let z_max: i32 = *old_state.keys().map(|(z, _w)| z).max().unwrap();
    let w_min: i32 = *old_state.keys().map(|(_z, w)| w).min().unwrap();
    let w_max: i32 = *old_state.keys().map(|(_z, w)| w).max().unwrap();

    let orig_rowlen: usize = old_state[&(0, 0)].len();
    let orig_collen: usize = old_state[&(0, 0)][0].len();

    for w in w_min..=w_max {
        for z in z_min..=z_max {
            let mut_slice_ref = old_state.entry((z, w)).or_insert(Vec::new());
    ...
    for w in w_min - 1..=w_max + 1 {
        old_state.insert(
            (z_min - 1, w),
            vec![vec!['.'; orig_collen + 2]; orig_rowlen + 2],
        );
        old_state.insert(
            (z_max + 1, w),
            vec![vec!['.'; orig_collen + 2]; orig_rowlen + 2],
...
fn count_active_neighbours(
    p: (usize, usize, i32, i32),
    old_state: &HashMap<(i32, i32), Vec<Vec<char>>>,
) -> usize {
        ...
        for z in p.2 - 1..=p.2 + 1 {
            for i in start_row..=p.0 + 1 {
                for j in start_col..=p.1 + 1 {
                    if let Some(char_val) = orig_state.get(i, j, z, w) {
                        if char_val == '#' && !(i == p.0 && j == p.1 && z == p.2 && w == p.3) {
                            active_count += 1;
                        }
                    }
                }
...
fn count_all_active(state: &HashMap<(i32, i32), Vec<Vec<char>>>) -> usize {
    let mut count = 0;
    let st = State(&state);
    for &z in state.keys() {
        for i in 0..state[&z].len() {
            for j in 0..state[&z][i].len() {
                if let Some(char_val) = st.get(i, j, z.0, z.1) {
                    if char_val == '#' {
                        count += 1;
                    }
                }
...
#[derive(PartialEq)]
struct State<'a>(&'a HashMap<(i32, i32), Vec<Vec<char>>>);

impl fmt::Display for State<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut keys = self.0.keys().collect::<Vec<&(i32, i32)>>();
        keys.sort_unstable();
        for z in keys {
            write!(f, "At z = {}, w = {}\n", z.0, z.1)?;
            for v in &self.0[z] {
...
impl State<'_> {
    fn get(&self, row: usize, col: usize, z: i32, w: i32) -> Option<char> {
        if let Some(slice) = self.0.get(&(z, w)) {
